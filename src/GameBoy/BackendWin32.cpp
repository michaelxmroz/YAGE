...
{
    m_window.m_data.m_width = width;
    m_window.m_data.m_height = height;
    m_windowThread = std::thread(std::ref(m_window));
}

void BackendWin32::CleanupWindow()
{
    m_window.m_data.m_run = false;
    m_windowThread.join();
...
void BackendWin32::CreateSurface(VkInstance instance, VkSurfaceKHR& surface)
{
    ...
    surfaceInfo.hwnd = m_window.m_data.m_hwnd;
...
{
    return !m_window.m_data.m_run;
}

void BackendWin32::SetWindowTitle(const char* title)
{
    SetWindowTextA(m_window.m_data.m_hwnd, title);
}

HWND* BackendWin32::GetWindowHandle()
{
    return &m_window.m_data.m_hwnd;
}
...
void BackendWin32::Window::operator()()
{
    ...
    HMODULE instanceID = GetModuleHandle(nullptr);
    m_data.m_hwnd = Win32Internal::CreateWin32Window(m_data.m_width, m_data.m_height, instanceID);

    MSG msg;
    while (GetMessage(&msg, NULL, 0, 0) > 0 && m_data.m_run)
    {
